...
const { getCoinBalance } = require("../../src/controllers/coinBalanceController"); // 'coinBalanceController'에서 'getCoinBalance' 함수를 가져옵니다.

const app = express(); // 'express' 인스턴스를 생성합니다.
app.use(express.json()); // express 애플리케이션에서 JSON 파싱 미들웨어를 사용하도록 설정합니다.
app.get("/coin-balance", getCoinBalance); // '/coinBalance' 경로에 대한 GET 요청을 처리하는 라우트 핸들러를 설정합니다.
...
describe("GET /coinBalance", () => {
  ...
  it("responds with the coin balance for a valid address", async () => {
    ...
    const response = await request(app)
      .get("/coin-balance") // '/coinBalance' 경로로 GET 요청을 보냅니다.
