...
const { handleError } = require('../controllers/errorController'); // 'errorController'에서 'handleError' 함수를 가져옵니다.

const app = express(); // 'express' 인스턴스를 생성합니다.
app.use(express.json()); // express 애플리케이션에서 JSON 파싱 미들웨어를 사용하도록 설정합니다.
app.get('/error', (req, res, next) => { // '/error' 경로에 대한 GET 요청을 처리하는 라우트 핸들러를 설정합니다.
