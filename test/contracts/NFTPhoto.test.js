...
contract('NFTPhoto', function (accounts) {
  ...
  describe('createPhotoNFT', function () {
    ...
    it('creates a new NFT and emits a TokenCreated event', async function () {
      ...
      const result = await contract.createPhotoNFT(tokenURI, tokenPrice, {
        from: accounts[0],
      });
      ...
      assert.equal(
        event.event,
        'TokenCreated',
        'TokenCreated event should be emitted'
      );
      // 이벤트의 'tokenURI'가 우리가 설정한 'tokenURI'와 같은지 확인합니다.
      assert.equal(
        event.args.tokenURI,
        tokenURI,
        'tokenURI should be the same'
      );
      // 이벤트의 'tokenPrice'가 우리가 설정한 'tokenPrice'와 같은지 확인합니다.
      assert.equal(
        event.args.tokenPrice.toNumber(),
        tokenPrice,
        'tokenPrice should be the same'
      );
    });
  });
